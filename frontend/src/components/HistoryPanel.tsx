...
export default function HistoryPanel({ onEntrySelect, onRerunAnalysis }: HistoryPanelProps) {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-slate-800 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 p-8">
  ...
  if (error) {
    return (
      <div className="bg-white dark:bg-slate-800 backdrop-blur-sm rounded-2xl shadow-xl border border-red-200 dark:border-red-800 p-8">
        <div>
          <h3 className="text-lg font-bold text-red-900 dark:text-red-400 mb-2">Failed to Load History</h3>
          <p className="text-red-700 dark:text-red-400">{error}</p>
          <button
            onClick={fetchHistory}
            className="mt-4 px-4 py-2 bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800 text-white font-medium rounded-lg transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100">Analysis History</h2>
        <button
          onClick={fetchHistory}
      ...
      <div className="mb-6 space-y-4">
        ...
        <input
          ...
          className="w-full px-4 py-3 border-2 border-gray-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900 focus:border-blue-400 dark:focus:border-blue-500 transition-all bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
        />
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Filter:</label>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="px-4 py-2 border-2 border-gray-200 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 focus:border-blue-400 dark:focus:border-blue-500 transition-all bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100"
            >
          ...
          {/* Entry Count */}
          <div className="ml-auto text-sm text-gray-600 dark:text-slate-400 font-medium">
      ...
      {filteredEntries.length === 0 ? (
        <div className="text-center py-16">
          <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-2">No History Found</h3>
        ...
        <div className="space-y-6">
          {sortedGroups.map(([problemSlug, groupEntries]) => (
            <div key={problemSlug} className="space-y-3">
              {groupByProblem && (
                <div className="px-4 py-2 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border-l-4 border-purple-500 rounded-lg">
                  <h3 className="text-lg font-bold text-purple-900 dark:text-purple-300">
                    {groupEntries[0].problem_title}
                  </h3>
                  <span className="text-sm font-semibold text-purple-700 dark:text-purple-400 bg-purple-200 dark:bg-purple-900/50 px-3 py-1 rounded-full">
